...
if(isset($_POST['level2app_submit']))
{
			...
			WHERE `FORM_ID,C,18` = '$_SESSION[Level2_Form_ID]';";
...
/*Fional Submit for Level 2--*/

if(isset($_POST['Level2_Final']))
{
       ...
       && !empty($_SESSION['ContactStatusLevel2']))
    {
        ...
        $sql = 'UPDATE `appl_status` SET `COMPLETED_DATE_L2,D` = "'.$Date.'", `COMPLETED_TIME_L2,C,8` = "'.$Time.'", `FINAL_STATUS,N,1` = "6"
              WHERE `FORM_ID,C,18` = "'.$_SESSION['Level2_Form_ID'].'";';
        ...
        if(mysqli_query($conn, $sql))
        {
            ...
            echo '<script>';
            echo 'alert("Level 2 Data Entry Completed Sucessfully!");';
            echo 'window.close();';
            echo '</script>';
        }
        else
